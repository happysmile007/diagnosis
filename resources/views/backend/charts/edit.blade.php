...
<div class="card">
    <div class="card-header">
        ...
        <div class="float-right">
            <a href="{{ route('admin.charts.index') }}" class="btn btn-success">View Charts</a>
        </div>
    </div>
    <div class="card-body">
        <div class="row">
        ...
        <div class="col-6">
            ...
            <div class="logic_tree question_tree" style="display:none">
                <ul class="treecontent">
                    @for ($q=0; $q<count($course_list); $q++) <li>
                        {{ $course_list[$q]['title'] }}
                        <ul>
                            @for ($r=0; $r<count($course_test_list[$q]); $r++) <li>
                                ...
                                <ul>
                                    ...
                                    <li class="type-{{ $question_item['questiontype'] }}" data-testid="{{$tk}}" data-value="{{strip_tags($question_item['question'])}}" onclick='selectMatrixQuestion(event,this.getAttribute("data-value"))'>{{ $question_item['id'] .'.'. strip_tags($question_item['question']) }}
                                        <span>[{{ $qtypes[$question_item['questiontype']] }}]</span>{!! $score_htm.$is_condition_logic !!}
                                    </li>
                                    @endforeach
                                    @endif
                                </ul>
            ...
            <div class="logic_tree textgroup_tree" style="display:none">
                <ul class="treecontent">
                    @for ($i=0;$i<count($course_list);$i++) <li>
                        {{ $course_list[$i]['title'] }} </a>
                        <ul>
                            @for ($j=0;$j<count($course_test_list[$i]);$j++) <li>
                                ...
                                <ul>
                                    ...
                                    @for ($k=0;$k<count($textgroup_list[$tk]);$k++) <li>
            ...
            <div class="row main-content" id="matrix_part">
                <div class="col-12" id="mat_set">
                    ...
                    <div id="col_panel" class="col-12" style="padding-top:10px;">
                        ...
                        @for($k=1;$k< count($current_content[0]);$k++) <div class="row">
                            <div class="col-2">
                ...
                <div id="row_panel" class="col-12" style="padding-top:10px;">
                    ...
                    @for($i =1;$i<count($current_content);$i++) <div class="row">
                        <div class="col-2">
                            <select class="form-control input-small select2me" data-placeholder="Select..." disabled>
                                <option>Single Input</option>
                                <option>Checkbox</option>
        ...
        <div class="col-12" style="padding-top:3vh;">
            ...
            <table id="real_matrix" width="100%">
                <tr>
                    ...
                    @for ($j=1;$j<count($current_content[0]);$j++) <td><input type="text" placeholder="" class="form-control head" value="{{ $current_content[0][$j] }}" disabled=""></td>
                        @endfor
                </tr>
                @for($i=1;$i<count($current_content);$i++) <tr>
                    <td width="15%"><input type="text" placeholder="" class="form-control head" value="{{ $current_content[$i][0] }}" disabled=""></td>
                    @for($j=1;$j<count($current_content[0]);$j++) <?php
                                                                    $idx = $j . "_" . $i;
                ...
                <div class="logic_condition row clone_condition" style="padding-top:10px;">
                    ...
                    <div class="col-5">
                        ...
                        <select class="form-control btn-warning" id="chart_type" placeholder="Options">
                            ...
                            ?>
                            @for($i=0;$i<count($types);$i++) @if ($current_chart->type == $i)
