...
class TestReportsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('backend.testreports.index', compact('courses', 'tests'));
    }
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->test_id == "")
        {
            if (isset($request->course_id) && $request->course_id != "") {
                ...
                ->whereIn('testreport_test.test_id', $test_ids)
    ...
    public function create()
    {
        ...
        $current_tests = Test::where('published', '=', 1)->select('id as test_id')->get();
        
        return view('backend.testreports.create', compact('tests', 'courses', 'current_tests'));
    }
    ...
    public function update(Request $request){
        ...
        if (! data_get($request->data, 'id')) {
            $last_id = DB::table('testreports')->insertGetId($values);
        } else {
            DB::table('testreports')
                ->where('id', $last_id = $request->data['id'])
                ->updateOrInsert(['id' => $last_id], $values);
        }

        DB::table('testreport_test')->whereIn('test_id', $tests)->delete();
    ...
    public function edit($id)
    {
        ...
        $tests = Test::where('published', '=', 1)->whereIn('course_id', $courses_ids)->select('title', 'id')->get();
        $current_tests =DB::table('testreport_test')->select('test_id')->where('testreport_id', $id)->get();
        return view('backend.testreports.edit', compact('testreport', 'courses', 'tests', 'current_tests'));
    ...
    public function student_report(){
        ...
        // $testreport =DB::table('testreports')->get();
         //dd($testreport);
